...
db <- dbConnect(SQLite(), dbname="database.sqlite") # trzeba sie polaczyc z baza;
	# polaczenie jest zamykane za pomoca 'dbDisconnect';
	# pierwszy argument to tzw. driver, czyli protokul laczenia;
	#
dbListTables(db)
...
fetch  <- function(db, query, n = -1) {
  result <- dbSendQuery(db, query) # The function ‘dbSendQuery’ only submits and synchronously executes
     # the SQL statement to the database engine.  It does _not_ extracts
     # any records - for that you need to use the function ‘dbFetch’, and
     # then you must call ‘dbClearResult’ when you finish fetching the records you need.
  data <- dbFetch(result, n) # Fetch the next ‘n’ elements (rows) from the result set and return
     # them as a data.frame.
  dbClearResult(result) # IMPORTANT: Frees all resources (local and remote) associated with a result
     # set.  It some cases (e.g., very large result sets) this can be a
     # critical step to avoid exhausting resources (memory, file
     # descriptors, etc.)
